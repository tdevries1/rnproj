...
Projection Approach for Estimating Risk-Neutral Expectations*](#citing):
...
The package is verified against the paper's original Matlab implementation
through a language-agnostic golden test suite (`tests/golden/*.json`,
generated by `matlab/export_goldens.m`): projection estimates match to
1e-10, Carr-Madan benchmarks to 1e-12, and the bivariate FX estimator to
1e-8. Pure-Python property tests additionally enforce the estimator's exact
invariants (spanned payoffs priced exactly, put-call parity, scale
invariance, full-rank automatic grids).

## Citing

If you use this package in academic work, please cite the paper:

> De Vries, Tjeerd (2026). *A Projection Approach for Estimating
> Risk-Neutral Expectations*. Working paper, HEC Paris.

```bibtex
@unpublished{devries2026projection,
  author = {De Vries, Tjeerd},
  title  = {A Projection Approach for Estimating Risk-Neutral Expectations},
  year   = {2026},
  note   = {Working paper, HEC Paris}
}
```
